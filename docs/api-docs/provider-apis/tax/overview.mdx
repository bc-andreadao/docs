...
If document submission is supported by the tax provider, the merchant can choose to select the **Submit Order Data** checkbox. This will enable document submission on their store, for supported transactions.
...
To test the tax provider connection, navigate to **Settings > Setup > Tax > _TaxProviderName_** in the control panel and click **Test Connection**. This selection will trigger a sample tax estimate request to be sent to the estimate URL provided by the tax provider. If the connection is unsuccessful, or there is an issue with the response, merchants can navigate to **Settings > Advanced > Store logs** in the control panel to view the error and its description to assist with triaging the issue.
